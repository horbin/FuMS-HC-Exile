...
// SpawnCrazy		- 	Scans the buildings spawned in the mission and if "Land_Sign_WarningUnexplodedAmmo_F" is found, it spawns a group of sicide bombers aroundinsrt it
// SpawnRads		- 	Scans the buildings spawned in the mission and if "Land_Device_disassembled_F" is found, it spawns a Sparky from it
...
///////////////// Alias Radiation Effects

{
	diag_log format ["<FuMS> MissionFXStart Rads check %1",_x];

	if (typeOf _x == "Land_Device_disassembled_F") then
	{
		private _radCenter = getPos _x;
		
		diag_log format ["<FuMS> MissionFXStart Radiation trigger found, starting script: Item :%1 | Pos:%2",_x,_radCenter];
...
{
	diag_log format ["<FuMS> MissionFXStart Nuke check %1",_x];

	if (typeOf _x == "Land_Device_assembled_F") then
	{
		private _nukeCenter = getPos _x;
		
		diag_log format ["<FuMS> MissionFXStart Nuke trigger found, starting script: Item :%1 | Pos:%2",_x,_nukeCenter];
